...
int dwrite(int r_fd,int w_fd,unsigned long long r_seek,unsigned long long w_seek,int read_size,int idx)
{
	int ret;
	char sp[1];
	char buf[1000];
...
int main(int argc,char **argv)
{
	...
	for(i=0;i<total_len;i++){
		...
		ret = dwrite(r_fd,w_fd,r_seek,w_seek,r_len,idx);
		if(ret < 0){
